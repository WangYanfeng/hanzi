...
function judgegroup(jname) {
    ...
    if (jname == table.user_name) {
        ...
        $('#jmid').css({ 'display': 'block' });
        // 比赛开始按钮
        if (!table.gamestart) $('#btnStart').css({ 'display': 'block' });
    } else {
        /* 比赛选手界面 */
    ...
    if (jname != "" && $('.judge').text() == "无裁判") {
        response_msg(new Array({ "uid": "裁判", "msg": "<font color='red'>我是裁判，若在比赛中对裁决有不同意见，可向我提出异议。\n谢谢！</font>" }));
    }
...
function judge_exit_group() {
    ...
    // 如果当前用户为选手，且已经选择了组，则不能显示我来当裁判按钮
    if (!table.groupinfo) {
...
function start_response(da) {
    ...
    // 发送给裁判
    $("#sExplain").text(da.explain);
    $("#sAnswer").text(da.answer);
    set_pinyin("#myTab_judge", da.pinyin);
    ...
    // 提示比赛开始信息
    if (da.operation == 'start') warningmsg("比赛已开始", "info");
    else warningmsg("继续下一题", "info");
    // 设置选手还没有提交答案
...
// 裁判宣布开始答题
function start_game() {
    if (table.gamestart) { warningmsg("比赛已经开始"); return; }
    if (!game_control.gamestart()) { warningmsg("人数尚未到齐"); return; }
...
function next_group() {
    // 选手还没有提交答案
    if (!game_control.issubmit_answer) { warningmsg('选手还未提交答案'); return; }
    // 没有发送评判结果
    if (!table.isjudge) { warningmsg('请发送评判结果'); return; }
    // 选手有异议
    if (table.is_objection) { warningmsg('选手对裁决有异议，请解决'); return; }
...
function judge_answer(param, final) {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            warningmsg("judge_answer error: " + textStatus, "error");
        }
    });
}