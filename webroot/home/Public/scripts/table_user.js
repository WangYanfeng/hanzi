...
function init_user_account() {
    ...
    for (var i = 0; i < groups.length; i++) {
        ...
        for (var j = 0; j < items.length; j++) {
            ...
            // 设置账号默认信息
            items.children(".userinfo").children(".account").text("");
            items.children(".userinfo").children("span").text("");
        }
    }
}
...
function managegroup(obj, pos) {
    ...
    // 选择组
    var item = $(obj);
    if (item.children(".userinfo").children(".account").text() != "") { warningmsg("已经有人了"); return; }
    
    // 注册用户信息
    register(parseInt(item.parent().attr("type")), pos);
}
...
// 回答问题
function answer() {
    if (!table.gamestart) { warningmsg("比赛尚未开始"); return; }
    if (!table.myturn) { warningmsg("暂未轮到你答题"); return; }
    if (event_mamage.submit_answer) { warningmsg("答案已提交，请稍等"); return; }
    if (!table.percent) { warningmsg("时间已用完"); return; }
    if (!table.gamestate) { warningmsg("你已不能继续比赛"); return; }
    // 上传回答的内容
    upload_img();
}
// 上传当前书写的内容
...
function upload_img() {
    ...
    $.post("/HanziGame/index.php?m=GameHall&a=upload", {
...
function answer_canvas(jname, imgurl, cuser_id) {
    ...
    if (cuser_id == table.user_name) {
        ...
        // 提示消息
        warningmsg("答案已经提交", "success");
    }

    // 区分裁判和用户
    var imgs = null;
    if (jname == table.user_name) {
        imgs = $("canvas[id^='sUserAnswer']");
    } else {
        imgs = $("canvas[id^='txtAnswer']");
    }
...
function del_user_account(gid, pos) {
    ...
    // 设置账号默认信息
    item.children(".userinfo").children(".account").text("");
    item.children(".userinfo").children("span").text("");
    // 移除头像
...
function set_user_account(gid, pos, account) {
    //item = $("#group" + gid + " div:nth-child(" + pos + ")");
    var item = $($("#group" + gid).children(".groupitem")[pos - 1]);

    // 添加账户
    item.children(".userinfo").children(".account").text(account);
...
function get_user_account(gid, pos) {
    return $("#group" + gid + " div:nth-child(" + pos + ")").children(".userinfo").children(".account").text();
}
// 获取第i组所有人员信息
function get_group_users(gid) {
    return $("#group" + gid).children(".groupitem").children(".userinfo").children(".account");
}
// 获取uid所在组中的元素
